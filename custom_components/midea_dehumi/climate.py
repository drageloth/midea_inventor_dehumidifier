...
from homeassistant.const import ( 
    CONF_USERNAME, CONF_PASSWORD, DEVICE_CLASS_HUMIDITY, TEMP_CELSIUS
)

import homeassistant.helpers.config_validation as cv

from homeassistant.components.climate import (  # noqa
    PLATFORM_SCHEMA,
    ClimateDevice
)
#ATTR_FAN_LIST -> ATTR_FAN_MODES // 
#ATTR_OPERATION_MODE - > ATTR_HVAC_MODE //
#ATTR_OPERATION_LIST -> ATTR_HVAC_MODES //
#SUPPORT_TARGET_HUMIDITY_LOW ->  ATTR_TARGET_TEMP_LOW,  ??
# SUPPORT_TARGET_HUMIDITY_HIGH, -> ATTR_TARGET_TEMP_HIGH,  ??
#ATTR_FAN_LIST -> ATTR_FAN_MODES
...
from homeassistant.components.climate.const import (
    ...
    SUPPORT_TARGET_HUMIDITY, ATTR_CURRENT_HUMIDITY, ATTR_FAN_MODE, 
    ATTR_FAN_MODES, ATTR_HUMIDITY, ATTR_HVAC_MODE,
    ATTR_HVAC_MODES, ATTR_TARGET_TEMP_LOW, ATTR_MIN_HUMIDITY,
    ATTR_MAX_HUMIDITY, SUPPORT_FAN_MODE, ATTR_FAN_MODES, ATTR_HVAC_MODES,
...
SUPPORT_FLAGS = SUPPORT_TARGET_HUMIDITY | SUPPORT_FAN_MODE
...
DEHUMI_HVAC_DICT = { 'HVAC_MODE_COOL' : 1, 'HVAC_MODE_HEAT' : 3, 'HVAC_MODE_FAN_ONLY' : 6, 'HVAC_MODE_AUTO' : 7, 'HVAC_MODE_DRY' : 8 }
DEHUMI_FAN_SPEED_DICT = { 'SILENT' : 40, 'MEDIUM' : 60, 'HIGH' : 80 }
DEHUMI_FAN_SPEED_LIST = [ 'SILENT', 'MEDIUM', 'HIGH' ]
DEHUMI_HVAC_LIST = [ 'HVAC_MODE_FAN_ONLY', 'HVAC_MODE_AUTO', 'HVAC_MODE_COOL', 'HVAC_MODE_HEAT', 'HVAC_MODE_DRY', 'HVAC_MODE_OFF' ]
...
class MideaDehumiDevice(ClimateDevice):
    ...
    @property
    def hvac_oper_list(self):
        return DEHUMI_HVAC_LIST
...
#            return None			#it is not possible to change operation mode when device is off
        return [HVAC_MODE_COOL, HVAC_MODE_AUTO, HVAC_MODE_FAN_ONLY, HVAC_MODE_HEAT, HVAC_MODE_DRY, HVAC_MODE_OFF]
#        return [self.hvac_oper_list]
    ...
    def hvac_mode(self):
        ...
        if not self.is_on:
            ...
            if (self._operation_list[self._setOperationMode - 1]) == "smart_mode-ion_off":
                return HVAC_MODE_AUTO
            elif (self._operation_list[self._setOperationMode - 1]) == "target_mode-ion_on":
                return HVAC_MODE_COOL
            elif (self._operation_list[self._setOperationMode - 1]) == "smart_mode-ion_on":
                return HVAC_MODE_HEAT
            elif (self._operation_list[self._setOperationMode - 1]) == "continuos_mode-ion_off":
                return HVAC_MODE_FAN_ONLY
            elif (self._operation_list[self._setOperationMode - 1]) == "dryer_mode-ion_off":
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        ...
        _LOGGER.debug("async_set_operation_mode called; current_mode=%s, new mode=%s", self._setOperationMode, hvac_mode)
        if hvac_mode == HVAC_MODE_OFF:
            ...
            if self.is_on:
                if hvac_mode == HVAC_MODE_COOL:
                    ...
                    hvac_mode = 8
#           mode = self._hvac_dict.get(hvac_mode, 0)
 #           if mode and mode != self._setOperationMode:
                ...
                self._client.deviceStatus._setMode = hvac_mode
